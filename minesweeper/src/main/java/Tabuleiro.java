...
//		System.out.println(retornaQuantidadeMinasEmVolta(campo) + " " + retornaCamposJaMarcadosEmVolta(campo).size());
	...
	public void atualizaCampos() {
		...
		for(int i = 0; i < tamanho; i++) {
			for(int j = 0; j < tamanho; j++) {
				Campo campo = matrizCampos[i][j];
				if(campo.getStatusCampo().equals(CAMPO_MARCADO) || campo.getStatusCampo().equals(CAMPO_VAZIO)) {
					continue;
				}
				...
				if(retornaCamposEmVoltaClicaveis(campo).size() == 0 ||
						retornaMinasMarcadasEmVolta(campo).size() == retornaQuantidadeMinasEmVoltaRestante(campo) ||
						campo.getStatusCampo().equals(CAMPO_VAZIO) ||
						campo.getStatusCampo().equals(CAMPO_MARCADO)
						) {
...
//				retornaCampo(numId).setStatusCampo(statusCampo);
	...
	public List<Campo> retornaCamposEmVoltaClicaveis(Campo campo) {
		...
		List<Campo> camposClicaveis = new ArrayList();
		List<Campo> camposEmVolta = new ArrayList();
	...
	public List<Campo> retornaCamposJaMarcadosEmVolta(Campo campo) {
		int posicaoX = campo.getPosicaoX();
		int posicaoY = campo.getPosicaoY();
		List<Campo> camposComMinaMarcada = new ArrayList();
		List<Campo> camposEmVolta = retornaCamposEmVolta(campo);
...
//		ArrayList<Campo> camposEmVolta = (ArrayList<Campo>) retornaCamposComNumeroEmVolta(campo);
